...
resource "aws_api_gateway_rest_api" "retouren_java_lambda_api" {
  name        = "retouren_java_lambda_api"
...
resource "aws_api_gateway_resource" "java_lambda_api_gateway" {
  rest_api_id = aws_api_gateway_rest_api.retouren_java_lambda_api.id
  parent_id   = aws_api_gateway_rest_api.retouren_java_lambda_api.root_resource_id
...
resource "aws_api_gateway_integration" "java_lambda_integration" {
  ...
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.retouren_java_lambda_function.invoke_arn
}
...
resource "aws_api_gateway_method" "java_lambda_method_root" {
  rest_api_id   = aws_api_gateway_rest_api.retouren_java_lambda_api.id
  resource_id   = aws_api_gateway_rest_api.retouren_java_lambda_api.root_resource_id
  http_method   = "ANY"
  authorization = "NONE"
}
